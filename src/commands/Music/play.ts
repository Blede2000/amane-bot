...
export class PlayCommand extends Command {
	...
	private getPlaylistEmbed(interactionOrMessage: Command.ChatInputCommandInteraction | Message, playlist: Playlist) {
		...
		trackPages.forEach((tracks) => {
			const songs = tracks.map((track) => {
				return `${playlist.tracks.indexOf(track) + 1}) [${track.title}](${track.url}) \n`;
			});
	...
	private chunkPlaylist(playlist: Playlist) {
		...
		return Array.from(new Array(Math.ceil(playlist.tracks.length / chunkCount)), (_, i) =>
			playlist.tracks.slice(i * chunkCount, i * chunkCount + chunkCount)
		);
	}
}
